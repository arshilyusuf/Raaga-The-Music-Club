...
const DEFAULT_INNER_GRADIENT =
  "linear-gradient(145deg,#252e08 0%,#252e08 100%)";
...
// Cloudinary Transformation Helper for Blur Placeholders
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      "--behind-glow-color": behindGlowColor ?? "rgba(37,46,8,0.67)",
  ...
  const glareStyle: React.CSSProperties = {
    ...
    backgroundImage: `radial-gradient(
      farthest-corner circle at var(--pointer-x) var(--pointer-y),
      hsl(74, 30%, 45%) 12%,       /* Lighter, softer olive green for the center shine */
    hsla(74, 60%, 15%, 0.5) 90%
    )`,
      ...
      <div ref={shellRef} className="relative z-[1] group">
        ...
        >
          ...
          >
            ...
            >
              {/* REPLACED: Standard img tag -> Next.js Image component */}
              {avatarUrl && (
                <Image
                  ...
                  height={600}
                  placeholder={
                    getCloudinaryBlurUrl(avatarUrl) ? "blur" : "empty"
                  }
                  blurDataURL={getCloudinaryBlurUrl(avatarUrl)}
                  // 1. Changed to absolute inset-0, w-full h-full, and object-cover
                  className="absolute inset-0 w-full h-full object-cover will-change-transform transition-transform duration-[120ms] ease-out"
                  style={{
                    transformOrigin: "50% 100%",
                    // 2. Removed '-50% +' from translateX since we removed left-1/2
                    transform:
              ...
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to top, rgba(0, 0, 0, 0.9) 0%, transparent 75%)",
                  borderRadius: cardRadius,
                }}
              />
              {showUserInfo && (
                <div
                  className="absolute z-[20] flex items-center justify-between backdrop-blur-[30px] border border-white/10 pointer-events-auto"
                  style={
                    {
                      "--ui-inset": "20px",
                      "--ui-radius-bias": "6px",
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      // ADDED: `relative` to allow 'fill' usage in next/image child
                      className="rounded-full overflow-hidden border border-white/10 flex-shrink-0 relative"
                      style={{ width: "48px", height: "48px" }}
                    >
                      {/* REPLACED: Mini avatar img -> Next.js Image */}
                      <Image
                        loader={cloudinaryLoader} // <-- ADD THIS
                        src={miniAvatarUrl || avatarUrl || ""}
                        alt={`${name || "User"} mini avatar`}
                        fill
                        sizes="48px"
                        placeholder={
                          getCloudinaryBlurUrl(miniAvatarUrl || avatarUrl)
                            ? "blur"
                            : "empty"
                        }
                        blurDataURL={getCloudinaryBlurUrl(
            ...
            <div
              className="max-h-full overflow-hidden relative z-[15]"
              style={{
                transform:
