...
export async function GET() {
  ...
  try {
    ...
    const dynamicGallery = academicYears.map((year) => {
      ...
      if (yearPhotos.length > 0) {
        ...
        const latestPhoto = yearPhotos.reduce((latest, current) => {
          ...
          return current.date.localeCompare(latest.date) > 0 ? current : latest;
        }, yearPhotos[0]);

        if (latestPhoto.date) {
          // Explicitly split elements to bypass local UTC offset rendering errors
          const [yearPart, monthPart, dayPart] = latestPhoto.date.split("-").map(Number);
